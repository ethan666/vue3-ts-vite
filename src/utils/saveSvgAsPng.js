...
export function svgAsDataUri(el, options, done) {
  ...
  return prepareSvg(el, options).then(function (_ref4) {
    ...
    var svgXml =
      "data:image/svg+xml;base64," + window.btoa(reEncode(doctype + src));
...
const prepareSvg = function (el, options, done) {
  requireDomNode(el);
...
export function svgAsPngUri(el, options, done) {
  ...
  var _ref5 = options || {},
    ...
    encoderType =
      _ref5$encoderType === undefined ? "image/png" : _ref5$encoderType,
    _ref5$encoderOptions = _ref5.encoderOptions,
    encoderOptions =
      _ref5$encoderOptions === undefined ? 0.8 : _ref5$encoderOptions,
    canvg = _ref5.canvg;
  ...
  if (canvg) return prepareSvg(el, options).then(convertToPng);
    ...
    return svgAsDataUri(el, options).then(function (uri) {
      return new Promise(function (resolve, reject) {
        var image = new Image();
